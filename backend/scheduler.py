...
def reload() -> None:
    ...
    # ── Watcher feed-retention cleanup (issue_local_008) ───────────────────
    # One interval job per watcher (regardless of enabled state) that trims its
    # stored feed down to max_feed_events. Immediate inserts keep up to the
    # global watcher_max_events hard limit; this enforces the per-watcher cap.
    try:
        from backend.db.watchers import list_watchers_cleanup_sync, run_watcher_cleanup

        for w in list_watchers_cleanup_sync():
            seconds = max(int(w["cleanup_interval_sec"]), 10)
            scheduler.add_job(
                run_watcher_cleanup,
                trigger="interval",
                seconds=seconds,
                args=[w["id"]],
                id=f"watcher_cleanup__{w['id']}",
                replace_existing=True,
            )
            logger.info("Scheduled watcher cleanup '%s' every %ds", w["id"], seconds)
    except Exception as exc:  # pragma: no cover — defensive
        logger.warning("Watcher cleanup scheduling skipped: %s", exc)
